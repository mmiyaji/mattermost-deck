...
- スレッド、検索結果、ピン留め投稿の表示時に、右ペインの実測幅を同じ描画フレームでDeckから差し引き、段階的なサイズ変更や本文の一時的な被覆を防ぎながら開く前のメイン表示幅を維持
- Deckが一時的に縮小されている間もマウス／キーボードで幅を変更でき、選択した幅を手動上書きとして保持
- レイアウト監視対象をMattermostのルートと正式な右ペインに限定し、大量の検索結果を読み込んでも監視処理や保持対象が増えないよう改善
- 配布ビルドをReact productionランタイムへ変更し、全体Profilerを上限付きレンダー診断へ置換して、長時間利用時のUser Timing蓄積を防止
- 384件のMattermost投稿と数万回の右ペインMutationを使う固定seedの20分耐久テストを追加し、Mattermost 9.5.11でrendererクラッシュ／OOMシグナルなしを確認
