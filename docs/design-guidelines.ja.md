...
- 保存済みまたは新しくドラッグした手動幅は明示的な上書きとして扱い、Mattermost を最低 320 px 残す範囲まで Deck を拡大できるようにする。設定値による自動幅では引き続き 720 px を確保する
- Mattermost の右ペイン（スレッド、検索結果、ピン留め投稿など）表示中は、その実測幅を通常のレスポンシブ Deck 幅から同じ描画フレームで差し引き、開く前のメイン表示幅を維持する。残る Deck 幅が 280 px 未満なら 52 px へ折り畳み、終了時に指定幅と水平スクロール位置を復元する
...
直近の同期ヒントはノイズを増やしすぎないこと。

- 長いリクエスト文は短いラベルへ要約する
- 同じ内容が連続する場合は `x3` のように集約する
- 通常運用で横目に見られる密度に抑える
...
返信は permalink やスレッド表示を優先し、折りたたまれたスレッド内にしかない投稿でも安定して開ける経路を選ぶ。
...
- 一度空状態が確定した後のバックグラウンド再読込では、空状態を保ったまま更新し、ローディングを点滅させない
...
- 通常モードでは標準レイアウトを保ちつつ、同じ人の近接投稿だけ縦間隔を少し詰めてもよい
- コンパクトモードではカードを使わない高密度な行表示とし、`時刻 投稿者: 本文` に近い見え方を優先する
- コンパクトモードの投稿者名はユーザーごとに安定した色を使ってよく、自分は現在テーマのアクセント色を使う
- 返信投稿は通常投稿と見分けられる視覚的手掛かりを残す
