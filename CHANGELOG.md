...
- Replaced the timeout-only PWA fallback with install-state checks and browser-specific manual instructions localized in all supported languages.
- Completed German, French, Japanese, and Simplified Chinese coverage for Deck, diagnostics, performance, popup, and release-note messages.
- Replaced hard-coded pane states and controls with actionable localized guidance and added visible popup errors when Mattermost cannot be opened.
- Standardized Docker E2E tests on the state file generated by `mm95:start` and added coverage for locale switching and delayed PWA install events.
