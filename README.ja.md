...
- 多言語 UI 対応：日本語、英語、ドイツ語、中国語（簡体字）、フランス語
- すべてのドロップダウン選択で ↑ / ↓ / Enter / Escape キーボード操作に対応
...
ロケールファイルは `src/ui/locales/` にあります。新しい言語を追加するには：

1. `en.json` を新ファイル（例: `ko.json`）としてコピーし、値を翻訳します。
2. `src/ui/i18n.ts` でインポートしてロケールコードで登録します。
3. `src/ui/settings.ts` の `DeckLanguage` と `normaliseLanguage` にロケールコードを追加します。
4. `src/options/index.tsx` の `languageOptions` に言語オプションを追加します。
